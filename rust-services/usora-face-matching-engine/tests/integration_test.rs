...
fn create_test_face() -> DetectedFace {
    DetectedFace {
        bbox: BBox { x1: 40.0, y1: 40.0, x2: 160.0, y2: 160.0 },
        landmarks: vec![
            [70.0, 80.0], [130.0, 80.0], [100.0, 110.0],
            [80.0, 130.0], [120.0, 130.0],
        ],
        confidence: 0.95,
        quality_score: None,
        face_angle: 0.0,
...
async fn test_iou_computation() {
    ...
    let d = BBox { x1: 5.0, y1: 0.0, x2: 15.0, y2: 10.0 };
...
async fn test_brightness_computation() {
    let white = DynamicImage::ImageRgb8(RgbImage::from_pixel(100, 100, image::Rgb([255, 255, 255])));
...
async fn test_contrast_computation() {
    let uniform = DynamicImage::ImageRgb8(RgbImage::from_pixel(100, 100, image::Rgb([128, 128, 128])));
...
async fn test_bbox_methods() {
    let bbox = BBox { x1: 10.0, y1: 20.0, x2: 110.0, y2: 120.0 };
...
async fn test_face_centered_check() {
    let face = DetectedFace {
        bbox: BBox { x1: 350.0, y1: 200.0, x2: 450.0, y2: 400.0 },
        landmarks: vec![],
        confidence: 0.95,
        quality_score: None,
        face_angle: 0.0,
