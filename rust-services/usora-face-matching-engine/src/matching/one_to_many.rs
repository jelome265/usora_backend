...
pub struct FaissMatcher {
    indices: Mutex<HashMap<String, faiss::index::IndexImpl>>,
...
impl FaissMatcher {
    pub fn new(config: &FaissConfig, threshold: f64, top_k_default: usize) -> Result<Self> {
        let index: faiss::index::IndexImpl = if config.index_path.exists() {
            info!(path = %config.index_path.display(), "Loading existing FAISS index");
            faiss::read_index(&config.index_path.to_string_lossy())
            ...
            faiss::IndexFlatIP::new(config.dimension as i32)?.into_index()
        };
    ...
    fn get_or_create_tenant_index<'a>(
        indices: &'a mut HashMap<String, faiss::index::IndexImpl>,
        tenant_id: &str,
        dimension: usize,
    ) -> Result<&'a mut faiss::index::IndexImpl> {
        if !indices.contains_key(tenant_id) {
            let flat = faiss::IndexFlatIP::new(dimension as i32)?;
            indices.insert(tenant_id.to_string(), flat.into_index());
        }
        Ok(indices
            .get_mut(tenant_id)
